...
## Fixed
* Fixed invite link tracking for User Joined audit log events
* Fixed an issue where Twitch live stream alerts were not being sent
...
## Changed
* The `/rp ship` command now uses a seed system so that if you ship the same users multiple times, they will receive the same compatibility.
